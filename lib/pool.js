...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    function SetupJobManager() {
        ...
        _this.jobManager.on('flagJob', function(jobId) {
	        _this.jobManager.setJobState(jobId, 0);
        }).on('submitJob', function(jobId) {
	        _this.jobManager.setJobState(jobId, 1);
        }).on('resolveJob', function(jobId, jobState) {
	        _this.jobManager.setJobState(jobId, 2);
            ...
            var currentJob = _this.jobManager.currentJob;

	        if (currentJob) {
		        currentJob = currentJob.getJobParams();
 	        }

            var shareJobId = shareData.job;
	        var currentJobId = currentJob ? currentJob[0] : shareJobId;

            var isValidBlock = !!blockHex;
            var isSyncedJob = shareJobId == currentJobId;
            var isValidShare = !shareData.error && isSyncedJob;
            ...
            */
            if (!isValidBlock || !isSyncedJob)
                emitShare();
            else {               
		        var jobState = _this.jobManager.getJobState(jobId);

                if (jobState == 1 || jobState == 2) {
                    emitWarningLog('Warning, ignored duplicate submit block'); // + blockHex); //<< blockHex could be huge
                } else {
                    _this.jobManager.emit('submitJob', shareJobId);
                    ...
                    SubmitBlock(blockHex, function () {
                        CheckBlockAccepted(shareData.blockHash, function (isAccepted, tx) {
                            ...
                            if (isValidBlock === true) {
                                _this.jobManager.emit('resolveJob', shareJobId);
                                
                                shareData.txHash = tx;
                            } else {
                                _this.jobManager.emit('flagJob', shareJobId);
